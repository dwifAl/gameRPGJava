...
public class Toko {
    ...
    public void prosesMenu(ArrayList<Item> arrItem, int kode){
        ...
        if (subPil == 0){
        ...
        else if (subPil > objItem.getArrTransaksi().size() || subPil < 0){
            ...
            if (pilihanItem != 0 && itemPilihan != null){
                if (pilihanItem > itemPilihan.size() || pilihanItem < 0){
                    System.out.println("Masukkan tidak tersedia");
                }
                else{
                    Item pil = itemPilihan.get(pilihanItem-1);
                    transaksiItem(pil, subPil);
                }
            } else if (pilihanItem != 0 && itemPilihan == null) {
    ...
    public void menu(ArrayList<Item> arrItem) {
        ...
        else if (pilihan > this.arrMenuToko.size() || pilihan < 0){
            System.out.println("Pilihan yang anda masukan tidak tersedia");
        }
        else{
